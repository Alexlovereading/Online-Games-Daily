...
/**
 * Directly derived and adapted from jinhucheung/2048-react.
 * Source: https://github.com/jinhucheung/2048-react
 * Commit: 63c77f4bb6b0b902c0277b4fec7aea12b65cdc75
 * Copyright (c) 2018 Jin Hu Cheung. MIT License; see /licenses and /NOTICE.
 * Based on 2048 game by Gabriele Cirulli (gabrielecirulli/2048, MIT).
 * Changes: built React component shell; added UTC daily seed for initial board,
 * touch-swipe controls, localStorage progress, shared stats/analytics,
...
export const GAME_2048_SLUG = "2048";
const BEST_KEY = "dgh:2048:best";
...
function buildInitialBoard(dk: string): Board {
  const seed = getDailySeed(dk, GAME_2048_SLUG);
  return initBoard(makeSeededRng(seed));
}

function tileClass(val: number): string {
  if (val === 0) return "g2048-tile g2048-empty";
  return `g2048-tile g2048-tile-${Math.min(val, 2048)}`;
}

export function Daily2048Game({ initialDateKey }: { initialDateKey: string }) {
  ...
  useEffect(() => {
    ...
    setStats(loadStats(GAME_2048_SLUG));
    setReady(true);
  }, []);
  ...
  const handleMove = useCallback(
    (direction: "up" | "down" | "left" | "right") => {
      ...
      if (!started.current && dateKeyRef.current) {
        ...
        trackEvent({ name: "game_start", slug: GAME_2048_SLUG, dateKey: dateKeyRef.current });
      }
      ...
      if (justWon) {
        ...
        setWon(true);
        persistProgress();
        const updated = completeGame(GAME_2048_SLUG, "win", dateKeyRef.current);
        setStats(updated);
        trackEvent(createGameCompleteEvent(GAME_2048_SLUG, dateKeyRef.current, "win"));
        void submitLeaderboardScore(GAME_2048_SLUG, dateKeyRef.current, newScore);
      } else if (gameOver && !overRef.current) {
        overRef.current = true;
        setOver(true);
        persistProgress();
        const updated = completeGame(GAME_2048_SLUG, "lose", dateKeyRef.current);
        setStats(updated);
        trackEvent(createGameCompleteEvent(GAME_2048_SLUG, dateKeyRef.current, "lose"));
        void submitLeaderboardScore(GAME_2048_SLUG, dateKeyRef.current, newScore);
      } else {
        persistProgress();
      }
  ...
  return (
    <section
      ...
      aria-label="Daily 2048"
      data-ready={ready}
      onTouchStart={handleTouchStart}
      onTouchEnd={handleTouchEnd}
    >
      <div className="g2048-header">
        <div className="g2048-title-row">
          <span className="g2048-logo">2048</span>
          <div className="g2048-scores">
            <div className="g2048-score-box" aria-label={`Score: ${score}`}>
              <span>Score</span>
              <strong>{score}</strong>
            </div>
            <div className="g2048-score-box" aria-label={`Best: ${best}`}>
              <span>Best</span>
              <strong>{best}</strong>
            </div>
          </div>
        ...
        <div className="g2048-meta">
          <p>Today&apos;s board · <time dateTime={dateKey || initialDateKey}>{dateKey || initialDateKey} (UTC)</time></p>
          <button className="g2048-new-btn" onClick={resetToday} aria-label="Restart today's board">
            New Game
          </button>
        </div>
      </div>
      ...
      <div
        ...
        role="grid"
        aria-label="2048 game board, 4 rows by 4 columns"
      >
            ...
            {rowTiles.map((val, c) => {
              ...
              return (
                <div
                  ...
                  role="gridcell"
                  aria-label={val ? `Tile ${val}` : "Empty cell"}
                >
                  {val !== 0 ? val : ""}
                </div>
              );
      ...
      <div className="g2048-controls" aria-label="Directional controls">
        <button className="g2048-ctrl-btn" onClick={() => handleMove("up")} aria-label="Move up">↑</button>
        <div className="g2048-ctrl-row">
          <button className="g2048-ctrl-btn" onClick={() => handleMove("left")} aria-label="Move left">←</button>
          <button className="g2048-ctrl-btn" onClick={() => handleMove("down")} aria-label="Move down">↓</button>
          <button className="g2048-ctrl-btn" onClick={() => handleMove("right")} aria-label="Move right">→</button>
        </div>
      </div>

      <dl className="g2048-stats" aria-label="Your 2048 statistics">
      ...
      {won && !wonContinue && (
        <div className="g2048-overlay g2048-won" role="alert" aria-live="assertive">
          <p>You reached 2048!</p>
          <div className="g2048-overlay-btns">
            <button onClick={() => { wonContinueRef.current = true; setWonContinue(true); }}>Keep going</button>
            <button onClick={resetToday}>New game</button>
          </div>
        </div>
      )}

      {over && (
        <div className="g2048-overlay g2048-over" role="alert" aria-live="assertive">
          <p>Game over · Score: {score}</p>
          <button onClick={resetToday}>Try again</button>
        </div>
      )}

      <p className="g2048-hint">Use arrow keys or swipe to slide tiles.</p>
    </section>
  );
}
